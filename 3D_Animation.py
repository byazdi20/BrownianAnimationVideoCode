# Jupyter notebook lets us make on-the-fly animation simulation easily
# We will use those graphical capabilities
# Very important for animating these foraging and boids simulations

%matplotlib nbagg
import numpy as np # import numpy library as np
import matplotlib.pyplot as plt # import pyplot library as plt
import matplotlib.mlab as mlab # import mlab module to use MATLAB commands with the same
import matplotlib.animation as animation # import animation modules from matplotlib
from mpl_toolkits.mplot3d import Axes3D # import Axes3D from mpl_toolkits.mplot3d
plt.style.use('ggplot') # use "ggplot" style for graphs

def init(): #used to initialize animation. necessary for FuncAnimation packages
    ...
    particles.set_data([],[]) # set position current (x,y) position data for all particle
    particles.set_3d_properties([]) # add current z data of particles to get 3d plot
    return particles,title,line # return listed objects that will be drawn by FuncAn

def animate(i): #function that iterates once and updates function values
    global R,V,W,Rs,Vs,Ws,time # define global variables
    time[i]=i*dt # store time in each step in an array time
    W = std*np.random.randn(nump,dim) # generate an array of random forces accordingly to
    ...
    particles.set_data(R[:,0],R[:,1]) # set the current position of all the particles in
...
anim.save('3D_Brownian_Motion.mp4',fps=50,dpi=100,writer="ffmpeg")
